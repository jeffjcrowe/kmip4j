...
@Entity
@DiscriminatorValue("Protect Stop Date")
public class ProtectStopDate extends Attribute {
	
	public ProtectStopDate(){
		super("Protect Stop Date", "ProtectStopDate", "DateTime");
		this.values = new ArrayList<KLMSAttributeValue>();
		this.values.add(new KLMSAttributeValue("DateTime", "ProtectStopDate"));
		this.values.get(0).setName(this.getAttributeName());
	}
	
	public ProtectStopDate(String value){
		this();
